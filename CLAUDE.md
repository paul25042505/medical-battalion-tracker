...
3. 在 `RELEASE_NOTES`（`index.html` 內，`FEEDBACK_CATEGORY_LABEL` 附近）補一筆
   當天的異動項目。結構是 `{ date, groups: [{ category, items }, ...] }`——
   `items` 是條列式短句陣列（畫面上排成清單），不要寫成一大段連續文字擠在
   一起看不清楚；每個異動項目要歸到一個 `category`（例如「患者紀錄」
   「急診待命」「任務管理」「介面優化」「推播通知」，沿用當天已有的分類，
   除非真的沒有合適的才新增），同一天有多筆出貨：同類別就加進同一個
   `category` 的 `items`，新類別就在該日期的 `groups` 陣列後面加一個新的
   `{ category, items }`；日期本身不要重複開新的。這樣「關於系統 → 更新
   紀錄」才會跟著實際出貨內容走，而且同一天出好幾次貨也不會變成一長串沒
   有結構的清單。
